...
public class Second {
private SecondaryIndex[] sI = new SecondaryIndex[100600];
...
public void add(){
 ...
 try{			
			RandomAccessFile recordfile = new RandomAccessFile ("original	1.csv","rw");
	...
	public void sortIndex() {
		...
		for(int i=0; i<reccount; i++)
		    {	
				for(int j=i+1; j<reccount; j++) {
					if(sI[i].getbName().compareTo(sI[j].getbName())  > 0)
		            {
		                temp.setbName(sI[i].getbName()); 
				        temp.setRecPos(sI[i].getRecPos());
				
			        	sI[i].setbName(sI[j].getbName());
			        	sI[i].setRecPos(sI[j].getRecPos());
				
			        	sI[j].setbName(temp.getbName());
		...
		public void search(){
             ...
             int oripos = binarySearch(sI, 0, reccount-1, bName);
             ...
             do {
                 ...
                 pos--;
                 if (pos < 0) { break;}
             }while(sI[pos].getbName().compareTo(bName)==0);
             
             pos = oripos + 1 ;
          // if (pos > reccount-1) { return;}
             
             while(sI[pos].getbName().compareTo(bName)==0 && pos > reccount-1) {
                 readFile(pos);
                 pos++;
 ...
 public void readFile(int pos) {
	 ...
	 if (pos == -1) {
            ...
            try {
                ...
                try {
                	String jl=sI[pos].getRecPos().trim();
        ...
        int binarySearch(SecondaryIndex s[], int l, int r, String bName) {
 ...
 long       pos=hey.getFilePointer();
        while((line = hey.readLine())!=null)
        {
            ...
            indexfile.writeBytes(columns[2]+","+pos+" \n");
...
public   void delete() throws IOException {
     ...
     int oripos = binarySearch(sI, 0, reccount-1, book_Name);
     ...
     do {
         ...
         pos--;
         if (pos < 0) { break;}
     }while(sI[pos].getbName().compareTo(book_Name)==0);
         
     
     pos = oripos + 1 ;
     
   //  if (pos > reccount-1) { return;}
     while(sI[pos].getbName().compareTo(book_Name)==0 && pos > reccount-1){
         readFile(pos);
...
public void markDelete(String b_Name,int pos) {
     ...
     String s=sI[pos].getRecPos().trim();
                 ...
                 while(st.hasMoreTokens()) {
                  ...
                  //   System.out.println("indexPos: " + indexPos);
                    // System.out.println("getrecpos: " + Long.parseLong(sI[pos].getRecPos()));
                     String t=sI[pos].getRecPos().trim();
